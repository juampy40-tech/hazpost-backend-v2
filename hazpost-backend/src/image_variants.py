...
def _render_basic_overlay(image, overlay_params=None):
    ...
    headline_font = _load_default_font(max(110, int(width * 0.13)))
    accent_font = _load_default_font(max(140, int(width * 0.16)))
    ...
    current_y = band_top + max(
        int(height * 0.05),
        int((band_height - total_text_height) / 2)
    )
